...
name: threat-scan
description: >-
  Run a diff-scoped security pass by inventorying touched trust boundaries and
  checking validation, authorization, bounding, destination encoding, leaks,
  secrets, cryptography, deserialization, supply chain, permissions, and
  security defaults. Use when a branch is security-relevant or the user asks
  for a threat scan.
---
# Threat Scan
...
**Read `$challenge`'s `SKILL.md` before parsing**
(`~/.codex/skills/challenge/SKILL.md` in a deployed tree) and apply its *Argument
