---
description: Record a solved problem as a durable solution doc plus a vestige recall pointer
argument-hint: '[optional hint: which problem from this session to record]'
allowed-tools: Read, Glob, Grep, Write, Edit, Bash(git status:*), Bash(git diff:*), Bash(git add:*), Bash(git commit:*), Bash(git branch:*), Bash(git rev-parse:*), Bash(git symbolic-ref:*), Bash(rg:*), Bash(mkdir:*), mcp__vestige__smart_ingest, mcp__vestige__search
---
...
From the current session (and `$ARGUMENTS` as a hint, if given), identify the problem
...
truth either way. Do not fail the command over a missing pointer.
