---
description: Adversarial review of files or git changes — challenges the approach, hidden assumptions, and failure modes
argument-hint: '[paths-or-globs ...] [--base <ref>] [--working-tree] [--json] [--out <path>] [focus text ...]'
allowed-tools: Read, Glob, Grep, Bash(git diff:*), Bash(git status:*), Bash(git merge-base:*), Write
---
...
command is almost always one step inside a larger workflow (e.g. a review loop).
...
case-sensitive; the start of `$ARGUMENTS` counts as a line start. So `CHARTER`, `   CHARTER (…)`,
...
- a candidate token was present but failed to resolve (a typo, a file deleted since the command
...
When `--json` is present, the command's **output artifact** is exactly this JSON object — no surrounding markdown or commentary *in the artifact*. Emitting it does not end your turn; resume the calling workflow afterward (see "Caller contract" below).
...
loop that re-runs this command until the verdict is `approve`). After emitting the
verdict:
...
- Do **not** end your turn merely because a verdict was produced. A
  ...
  way, there is more work after this command.
