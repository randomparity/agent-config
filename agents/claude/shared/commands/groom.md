---
description: Proactive maintenance sweep — stale issues, dependency freshness, supply-chain advisories, and deferral records past review-by; files work, never does it
argument-hint: '(no args — sweeps the repo)   (or: a single sweep letter, e.g. D)'
allowed-tools: Read, Grep, Glob, Bash(gh repo view:*), Bash(gh issue list:*), Bash(gh issue view:*), Bash(gh pr list:*), Bash(gh label list:*), Bash(gh api repos/*/issues/*/timeline*), Bash(git log:*), Bash(git status:*), Bash(osv-scanner:*)
---

# Groom the Repository

Sweep the four maintenance signals nothing else in this command set watches, and turn each into
...
`allowed-tools` above pre-approves **reads only**, deliberately. `/groom` is the one command
here that may run unattended, so every write it proposes — filing an issue, labelling, closing,
pushing a branch, opening the PR — reaches the operator as a prompt rather than a pre-granted
permission (ADR 0017: the list grants, it never restricts). The `gh api` entry is scoped to the
timeline path rather than `gh api:*`, which would pre-approve every REST write including a
merge; if a host's matcher does not honour the mid-path wildcard the effect is an extra prompt,
...
invoking `claude -p "/groom"`, and understand what that run can and cannot do first.
...
the reads-only `allowed-tools` list above is designed to force.
