---
description: Triage GitHub issues — analyze each and apply repo labels for work-selection; bootstrap a missing label set
argument-hint: '[issue-numbers ...]   (no args → sweep untriaged open issues)'
...
offer to bootstrap one. The command is **add-only** except within the
...
The label set the command checks for and can bootstrap:
...
`awaiting-merge`/`needs-human`) are written by the lifecycle commands, but this command's
...
- **Swap** within `priority:`/`status:`/`risk:`: when the issue already carries any
  ...
  only cases where the command removes a label.
...
1. **Resolve repo.** Run `gh repo view --json nameWithOwner --jq .nameWithOwner`
   ...
   call. Do not use `git remote` — this command's `allowed-tools` is `gh` only.
