---
description: Build with TDD — execute the implementation plan (subagent-driven or direct), then run the full guardrail suite
...
- **A plan exists and tasks are mostly independent:** execute it with
  ...
  code quality.
- **No plan because this is a trivial bugfix, or tasks are tightly coupled:**
...
`/review-loop`, `/simplify`, `/ship-pr` and `/merge-cleanup` follow. The skill
...
If no plan path is given in `$ARGUMENTS`, look for one under
`docs/superpowers/plans/`. If none exists and the change is non-trivial, stop
and tell the user to run `/design` first.
...
(not under `.claude/`, not any subdir). Whole-tree tooling (linters, type
...
- applicable `CLAUDE.md` / `AGENTS.md` conventions
- exact guardrail commands it must run before committing
- TDD requirements below
- the **subagent report contract** (`CLAUDE.md`) — the prompt must end with it so
...
command pipeline's canonical scale is `/challenge`'s `critical | high | medium | low`.
When a task-review finding is carried outward — into `/review-loop` or the `WORK:REVIEW`
summary `/work-issue` posts — convert it with the table in `challenge.md`, *Severity
...
note), so a post-compaction resume can recover them. Do **not** run `/compact`
