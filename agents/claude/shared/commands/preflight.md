---
description: Discover repo environment — base branch, guardrail commands, working-tree state, gh auth, parallel-run context — before touching code
---

# Preflight: Discover the Working Environment

Discover and record the repo environment before touching code or switching
branches. Record every finding in TodoWrite — later steps (`/design`,
...
outlives TodoWrite across a fresh-session resume.
...
Read `CLAUDE.md`, `AGENTS.md`, and any nested instruction files that apply to
the files you will touch. If those files are in a language with a
`~/.claude/languages/<lang>.md` reference (Python, TypeScript, Rust, Bash, GitHub
Actions), read that reference before writing code — it holds the strictness and
supply-chain rules that are no longer inline in `CLAUDE.md`.
...
Record the exact commands in TodoWrite. Note which checks CI hard-gates
