---
description: Implement a GitHub issue end-to-end: scope, design, TDD, adversarial-review loops, PR, CI, merge handoff, cleanup
argument-hint: <issue-number>
---

Implement GitHub issue **#$ARGUMENTS** end-to-end on a feature branch, following
this repo's conventions in `CLAUDE.md` / `AGENTS.md`, and drive it to a CI-green,
...
> **Single continuous task.** This command is one task from preflight through
> cleanup. It delegates to sub-commands (`/preflight`, `/design`, `/build-tdd`,
> `/review-loop`, `/simplify`, `/ship-pr`, `/merge-cleanup`), each of which is also
...
> only recovery path. Do **not** run `/compact` proactively; just keep them
...
Run `gh issue view $ARGUMENTS --json title,body,labels,comments` and follow linked
...
**Adopt an existing `/scope` report.** If the operator ran `/scope $ARGUMENTS` in this
...
create `feat/<short-slug>-$ARGUMENTS` off it. Never work on the default branch.
...
`.claude/`, not any subdir) — whole-tree tooling (linters, type checkers, test
...
Run `/design $ARGUMENTS` to write the spec + ADR, adversarial-review the spec,
...
`/design` (see `design.md` for their exact paths — the ADR lives under
...
returns full markdown inline, which puts a findings payload into this command's
...
- **widens a permission grant** — a command's `allowed-tools` (ADR 0017), a CI
...
Run `/simplify` to apply reuse, simplification, and efficiency cleanups to the
...
*pre-simplify* code, so if `/simplify` makes any change that could alter behavior
...
Run `/ship-pr $ARGUMENTS` to push the branch, create the PR, and drive it to
