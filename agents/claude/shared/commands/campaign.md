---
description: Orchestrate a set of GitHub issues to closed/merged — triage each (close if already fixed, else fix via /work-issue), review, simplify, serial-merge, and re-enqueue newly filed issues until the set is empty
argument-hint: '<issue-numbers | range | "query"> [completion notes]'
---
...
reimplementing the lifecycle. Follow this repo's conventions in `CLAUDE.md` /
`AGENTS.md`.

> **Single continuous task.** This command is one task from resolving the issue
> set through the last merge. It delegates to sub-commands (`/preflight`,
...
Parse `$ARGUMENTS` into a set of issue numbers. Support, in any combination:
...
can collide. Use `.claude/campaigns/<slug>.md`. Routing on the existing file — on
...
- **File with `Status: complete`** → the prior campaign under this selector
  ...
  name (`.claude/campaigns/<slug>-done-<UTC-timestamp>.md`, timestamp-suffixed so
...
`/campaign` runs against arbitrary repos, and many **track** `.claude/` — there
the manifest would land in a per-issue PR diff or trip `/preflight`'s dirty-tree
stop. So ensure `.claude/campaigns/` is in `.git/info/exclude` (per-clone, local,
...
stop with a named blocker rather than resuming on a partial queue. TodoWrite
mirrors the manifest for live progress display only; the manifest, not TodoWrite,
...
**Reconcile prior-run state first.** This command must survive being resumed
...
report contract in `CLAUDE.md`) — return only:
...
TodoWrite. Fill each issue's `ADR/migration #` and `File scope` columns. A genuine
...
collision the assignment prevents (`CLAUDE.md`: "prefer file-based state for
...
contract** (`CLAUDE.md`) — a ~1–2k-token summary (outcome, branch/PR ref, files
...
`Agent` tool, up to **5 per wave** (matching `/merge-dependabot`'s waves of 5), all
`Agent` calls in a **single message** per wave; subsequent waves start after the
...
operator-authorized branch (this command is the operator authorization). Merge one
...
to an in-scope issue), add a row to the manifest queue (and its TodoWrite mirror)
...
- **Its report names a blocker** — then it already posted the `WORK:TRAJECTORY` note and
  set `status:blocked`/`status:needs-human` (see `work-issue.md`, *On a Blocker*). Record
