---
description: Diff-scoped security pass — enumerate the trust boundaries a change touches and check what crosses each one
...
**Caller contract & constraints (front-loaded so they survive truncation).** This command is
...
**Read `challenge.md` from this command's own directory before parsing** (it sits beside
this file — `~/.claude/commands/challenge.md` in a deployed tree) and apply its *Argument
...
reconstructing it from memory. If `challenge.md` is genuinely absent, say so and fall back
