...
breaks the run with no recovery step the command specifies.

The `--unshallow` in the fallback is for a clone left behind by an earlier run of
this command: `/tmp/depbot-eval-*` persists between runs, a plain `git fetch` on a
...
- **`$MERGE_FLAG`** — the first merge method this repo allows, in this order:
  ...
  `merge-cleanup.md` and both `CLAUDE.md` files without needing an exception, and
...
Launch up to 5 subagents in parallel using the `Agent` tool. Each
call must use:
...
3. Verify the merge succeeded (gh pr merge produces no output on
   ...
   a refusal is the repo reporting something this command did not test.
