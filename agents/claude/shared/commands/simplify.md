---
description: Behavior-preserving simplification pass over recent changes — remove speculative abstraction, dead code, and cleverness; verify with the repo's guardrails
argument-hint: '[paths or PR/branch ref — default: working-tree + branch diff vs base]'
---
...
`$ARGUMENTS` overrides: explicit paths, a PR number, or a branch ref. Never widen scope
beyond what was asked — an unrelated file with ugly code gets flagged in the report, not
edited.

## What to remove or tighten

Apply the global standards (`~/.claude/CLAUDE.md`), in this priority order:
