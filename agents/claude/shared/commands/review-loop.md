---
description: Adversarial review loop — iterate /challenge until approve or 5 iterations, applying fixes between each pass
argument-hint: '<challenge-args> [focus text]'
---
...
standalone command and a subroutine of `/work-issue` and `/design`.

Raw arguments: `$ARGUMENTS`

The arguments are passed through to `/challenge` verbatim as the challenge
target and optional focus text. This command adds `--json --out <findings-path>`
...
deterministically from a hash of the **target and flag tokens** in `$ARGUMENTS` plus a
...
restated. If a caller typed a `CHARTER` block into `$ARGUMENTS`, strip it before
...
commands copied into repos that track `docs/` and even `.scratch/`. Pass the
...
- `challenge_args`: exact `/challenge` arguments, including paths, `--base`,
  `--working-tree`, or globs. This is `$ARGUMENTS`.
- `focus`: optional focus text appended after the target arguments. This is
  also part of `$ARGUMENTS` — challenge extracts it.
...
1. Run `/challenge` in a **subagent** with `--json --out <findings-path>
   $ARGUMENTS`, then the charter appended as a labeled trailing block:
   ...
   focus: <the focus text from $ARGUMENTS, restated>
   ```
   ...
   **The block has exactly those four fields.** The target and base are carried by
   ...
   older vendored `challenge.md` would target-classify it.
   ...
   **after** the strip in the paragraph below, never before. When the post-strip
   `$ARGUMENTS` carries a target or a mode flag, forward it unchanged. When it carries
   ...
   **Prefer naming the surface by component, not by path** ("the auth middleware",
   "this command file and its ADR"). This is a preference now, not a guard:
   `/challenge`'s stop rule makes a charter path safe, so precision no longer costs
   correctness. It stays recommended because `challenge.md` is vendored into other
   repos, and a run whose reviewer is an older copy without the stop rule still
   misparses a path.

   **A `CHARTER` block inside `$ARGUMENTS` is not a charter.** It is caller-supplied
   ...
   file (challenge.md's sole write exception); without `Write`, `--out` silently
...
Before or while running this command, the user may type a `/goal` (a Claude Code
