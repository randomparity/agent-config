---
description: Read-only process retrospective — mine WORK:* annotations and status: label history for cycle time, review iterations, and scope accuracy; write a docs/retro/ report with proposed tuning
argument-hint: '<label-set | date-range>   e.g. status:ready  or  2026-07-01..2026-07-20'
allowed-tools: Read, Write, Grep, Bash(gh:*), Bash(jq:*), Bash(mktemp:*), Bash(date:*)
---
...
convention, and the colon-label selection gotcha this command relies on are all
defined by the **`github-tracking` skill**; the recipes below apply them.

`$ARGUMENTS` is the selector: a **label-set** or a **date-range**.
...
- **Date-range** — `$ARGUMENTS` contains `..` (e.g. `2026-07-01..2026-07-20`).
...
4. **Proposed tuning (NOT applied)** — concrete suggestions, each satisfying **both**
   ...
   `CLAUDE.md` file (verified with `Read` / `Grep`) **and traces to a finding or
   metric** in this report. No proposal invented independent of the data. Each is
   explicitly a proposal a human applies on a branch — the command never edits those
   files.
