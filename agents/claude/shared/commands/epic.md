---
description: Interview a rough idea into a PRD epic; file decomposed sub-issues born triaged
argument-hint: '<rough feature idea>'
allowed-tools: Read, Grep, Glob, Bash(gh:*), Write, Agent
---

# Plan an Epic

Turn the rough idea in `$ARGUMENTS` into a filed **epic**: a PRD living in the epic
...
it; that one confirmation covers **every** write this command performs, including the
...
- **`gh` < 2.94.0** (no `--parent`): decompose mode's documented `sub_issues` API
  fallback applies; this command delegates rather than handling it.
