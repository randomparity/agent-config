...
**Refuse it if it nests.** If the native tool would place the worktree inside the repository tree (under `.claude/`, a project-local `.worktrees/`, or any other subdirectory of the working copy), do not use it. Go to Step 1b and run `git worktree add` against an external path yourself. Whole-tree tooling — linters, type checkers, test discovery, search — walks a nested worktree and then fails your commit on another agent's in-flight code. Harness-managed cleanup does not buy back that cost.
