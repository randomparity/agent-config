...
- **`blocked`/`needs-human` exit edges.** A human clears them by re-running `/work-issue N`
  (→ `in-progress`) or re-triaging (→ `ready`). The command that moves an issue *into*
...
A command must not assume a `status:` label exists. Before a transition, ensure-create the
