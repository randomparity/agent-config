...
**You are in dispatched mode when your instructions came from a curated command (`/design`, `/work-issue`) or from an orchestrator that dispatched you — not from a human typing in this turn.** Otherwise assume a human is present and follow the rest of this skill as written. Do not try to infer the mode: your caller states it. Mode is a property of the run, not of the skill, so every skill you invoke downstream inherits it. Name the resolved mode when you announce the skill.
