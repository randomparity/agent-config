---
name: codex-fleet
description: Offload high-volume, well-specified generation to parallel OpenAI Codex CLI workers. Use when a job splits into independent worker-sized tasks (content batches, test generation, bulk refactors) and Codex CLI is installed and logged in — Claude decomposes, briefs, spawns `codex exec` runs in parallel, and review-gates every output. Not for judgment-dense work Claude should do itself, or for coordinating Claude subagents on one complex change.
---

# Codex Fleet

Claude is the orchestrator — it decomposes, briefs, and reviews. Parallel
OpenAI Codex CLI workers are the fleet. Claude tokens buy judgment, Codex
tokens buy volume: a clear spec plus high volume goes to Codex; a fuzzy spec
or expensive-if-wrong work stays with Claude. Nothing ships unreviewed.
...
2. **Brief.** One markdown file per task in `briefs/`. Codex never sees the
   Claude conversation, so each brief is self-contained: job, inputs (file
...
4. **Review gate (Claude, mandatory).** Read every `out/` file against the
...
- Secrets never go into briefs or `AGENTS.md`; workers get file paths, not
  tokens.
- If output violates a hard ban, the fix is Claude's edit or a delta run —
  never "close enough".
