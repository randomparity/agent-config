...
RECORD_PROFILES="adr debt" ~/.claude/skills/decision-records/assets/migrate-records.sh
RECORD_PROFILES="adr debt" ~/.claude/skills/decision-records/assets/migrate-records.sh --write
...
Copy six files out of this skill's `assets/` directory. When the skill is deployed by
`install.sh` that is `~/.claude/skills/decision-records/assets/`; when you are working in
the repo that publishes it, `shared/skills/decision-records/assets/`. Resolve it before
copying rather than assuming either:
