...
| a `comm` that fails only on the skills-half comparison | exit 2 and the skills-half comparison message exactly. The existing mock exits 1 unconditionally and so faults inside the three-tree half, leaving the skills-half routing unasserted. The mock discriminates on its **operands** — the skills workspace files are distinctly named — and delegates otherwise, so it lands in exactly one half by construction, with no counter, no cross-process state, and no dependence on which half the script computes first. Delegation is the part with a trap: the mock runs with `$mock_bin` on `PATH`, so a body ending `exec comm "$@"` re-execs the mock in an unbounded loop — a CI job consumed to its timeout rather than a red row. Stripping `$mock_bin` from `PATH` only holds while it is `PATH`'s first element, so the helper instead resolves the real tool with `command -v` *before* `$mock_bin` is prepended and bakes in the absolute path, which is the shape `check-skill-layout-test.sh` already uses for its shims |
| a `sort` that fails only on the skills-half enumerated names | exit 2 and the skills-half sort message exactly, by the same operand discrimination and the same pre-resolved delegation |
