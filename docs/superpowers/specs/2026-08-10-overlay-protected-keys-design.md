...
anything else useful — and because `.[0]` is the base, it echoes a truncated prefix of the *base*
file, pointing the operator at the wrong input. Without the `length == 1` half, an overlay of two
concatenated objects — a stray appended block, two host fragments, an agent-generated file — passes
every later check while `.[1]` silently discards everything after the first document. That is a
silent discard of operator intent at this boundary, which is the failure class this change exists to
close, so it is a hole in the control rather than an accepted residual. The message distinguishes
...
8. **No overlay at all.** A run whose private overlay directory holds no `settings.overlay.json`
   ...
   `AGENT_CONFIG_PRIVATE_DIR`. It is also what catches a shape check misplaced above the
