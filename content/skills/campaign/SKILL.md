...
**The precondition binds to an agent this run dispatched.** A row you adopted on resume with its PR already open, and a row whose cleanup an earlier run deferred, have no such agent and never will — waiting on a notification that cannot arrive leaks them permanently. For those, observe instead: `git worktree list` showing the branch checked out nowhere means no holder, and a worktree whose `git status --porcelain` is empty was not being worked in. Read both before touching anything; neither requires performing the removal it authorizes. Anything else defers.
...
3. **Delete the branch** — worktree first, since a branch checked out in one cannot be deleted. `git branch -d` tests against your current `HEAD`, not against `origin/<BASE_BRANCH>`, so it refuses on the squash path and on any base you are not standing on. `-D` is permitted for the two cases item 1 proved merged, and for nothing else — never `-D` a branch that failed both tests.
...
The operator owns them from there, and today owns them alone. `$clean-branches` looks like the standing sweep and is not: it enumerates candidates by an upstream reading `[gone]`, and one of these branches carries no upstream at all until it is pushed, or one pointing at the base — neither is `[gone]`, so the sweep passes them over.
