...
The operator owns them from there, though no longer alone. `$clean-branches` enumerates every local branch and classifies on push evidence, so a merged worker branch reaches its delete rows in either setting of `deleteBranchOnMerge`. That collects a row deferred for **end of run not observed**; it does not resolve the other two reasons so much as agree with them — a row deferred for an unclean worktree hits the sweep's own dirty-worktree skip, and one whose branch did not land is reported as unmerged and never deleted. Read those skips as the second opinion they are: forcing past one with `--force` or `-D` discards the stranded work the deferral exists to protect. Two further leftovers it reports rather than collects — a branch whose commits never reached the remote, and one pushed without `-u` into a repo that *does* delete head branches on merge, or pushed to a remote not named `origin`; neither leaves local evidence under the branch's own name. Those stay the operator's by hand.

**Never point that sweep at a checkout that still holds a dispatched worker** — not merely one of yours. It has no liveness signal of its own, and its clean-`git status --porcelain` row is not one: that catches a worker which left modified or untracked files, not one which has committed and is mid-`push`. Its enumeration is repo-wide, so it reaches worktrees this campaign never created, and sweeping while any of them is live removes a directory its owner is standing in — the failure the observed-end-of-run precondition above exists to prevent, reached by another route. Your deferred rows are the worst case for it, being by construction the ones whose agent's end you never observed. So keep naming them in the report: the sweep is the operator's move once this campaign is over, its agents have stopped, and no other dispatched worker is in flight in the same clone — not a substitute for the report.
