...
The operator owns them from there, though no longer alone: `$clean-branches` classifies every local branch on push evidence, so a merged worker branch is now something it can collect rather than pass over. Don't read its verdicts off this report, though — it applies its own rules, and they are not step 6's. A row deferred for **end of run not observed** is the one it plainly collects; a row whose worktree was not clean hits its own dirty-worktree skip; and its squash-path test is a containment where item 1's is a SHA identity, so a row deferred for **branch did not land** may still be collected there. A branch whose commits reached no `origin` ref it never collects at all. Which of your rows fall where is the plan table it prints before its single confirmation, and its own skill — not this paragraph.

**Never point that sweep at a checkout that still holds a dispatched worker**, yours or anyone's. It has no liveness signal, its enumeration is repo-wide, and a clean `git status` says nothing about whether a process is inside a directory — so sweeping while any worker is live removes a worktree its owner is standing in, which is the failure the observed-end-of-run precondition above exists to prevent, reached by another route. Your deferred rows are the worst case for it, being by construction the ones whose agent's end you never observed. So keep naming them in the report: the sweep is the operator's move once the campaign is over and no dispatched worker is in flight in the same clone, not a substitute for the report.
