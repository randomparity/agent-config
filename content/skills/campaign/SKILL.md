...
The operator owns them from there, though no longer alone: `$clean-branches` classifies every local branch on push evidence, so a merged worker branch is now something it can collect rather than pass over. Don't read its verdicts off this report, though: it applies its own rules, not step 6's, and a row you deferred may be collected there, skipped there, or neither. What it will not do is delete a branch with no evidence it was ever pushed, however thoroughly its commits are in the base — that one stays the operator's by hand, and by hand is the only way it goes. Which of your rows fall where is the plan table it prints before its single confirmation; read that for the ignored-file inventory on each removal line, which is the one loss in the sweep nothing restores.

**Never point that sweep at a checkout while a campaign or a dispatched worker is in flight**, yours or anyone's — that is the constraint it states for itself. It has no liveness signal, and its enumeration is repo-wide, so it reaches worktrees nobody here dispatched. Its own dirty-worktree skip is not the reading allowed above: that one is taken about a row whose history you know, where the sweep applies it blind — and a worker that has committed and is mid-`push` reads clean to both. Sweeping while any worker is live therefore removes a worktree its owner is standing in, the failure the observed-end-of-run precondition above exists to prevent, reached by another route. Your rows deferred for **end of run not observed** are the worst case for it, being the ones you could not settle that question on yourself. So keep naming them in the report: the sweep is the operator's move once the campaign is over and nothing is in flight, not a substitute for the report.
