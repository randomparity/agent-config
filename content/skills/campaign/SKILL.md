...
- **List the changed files** — `gh pr diff <PR> --name-only`, on every PR, including one step 3 adopted without a scope assignment; those are the PRs you never dispatched and hold no subagent report for, so the list is worth more there, not less. Don't take it from `gh pr view --json files`: that field returns the first 100 paths with no truncation marker. A diff that succeeded and listed nothing blocks — nothing was changed, so nothing can be carrying the fix. A failure that survives a retry (the endpoint refuses very large diffs, deterministically) falls back to `git diff --name-only <base>...<head>` on the fetched head; if that fails too, hold rather than merge, since the merge is the irreversible half.
- **Compare that list against the issue's `File scope` cell from step 4.** A changed path is in scope when it equals a repo-relative path named in the cell, sits under a directory named there, or sits under the directory holding a glob entry. A cell still at `—` or holding nothing parseable as a path leaves nothing to compare against — record that, and read every path through the next bullet rather than treating an absent hint as a mismatch.
- **Changed files outside the scope do not block by themselves.** Step 4 assigns scope as a hint, and a correct fix routinely touches a file the plan didn't predict; a check that hard-blocks on any deviation fires on legitimate work and gets routed around. A file is accounted for when the PR body, a commit message, or the subagent's report ties it to **the assigned issue**; when it is the ADR index under `coupled` coupling below, or a file your own branch refresh or artifact regeneration touched, both of which this step mandates and neither of which names an issue; or when the operator already accepted it, per the hold below. Everything else is **unrelated** — hold that one merge for the operator's decision. A file tied to a *different* tracked issue is the case most needing that decision, not an exemption from it: merging it lands a sibling's work early and can auto-close a row still queued. A nonempty diff overlapping nothing in a populated scope is held the same way — a probably-stale hint, not proof the PR is wrong. Never split, revert, or cherry-pick inside the PR; that surgery is undefined here and risks discarding work.

Whether the PR *implements* its issue is not what any of this answers: the closing reference says which issue it claims, the file list says where it landed. Read the PR body, its stated acceptance criteria, and the `WORK:REVIEW` summary, and record what that read found — but hold only on the checkable triggers above, never on an unstated inability to confirm, or every reviewed PR becomes an operator's problem.

**A hold does not clear itself.** Post `WORK:TRAJECTORY`, set `status:needs-human`, record the row as blocked with the paths that caused it, and keep draining the rest of the queue. The PR stays green and mergeable throughout — nothing about it will change — so only an operator's decision lifts the hold, and step 3's `green + mergeable → ready-to-merge` reconciliation does not apply to a row carrying one. **That decision comes back on the issue, not in the manifest**: the operator accepts paths in a comment carrying one `Operator-accepted: <path>` line per path, and a path named on such a line is accounted for on every later pass. Nothing else in the comment counts, and nothing you wrote yourself ever does. GitHub is the parked state, so it is the released state too — the manifest is gitignored, keyed to one selector on one machine, and yours alone to write, so an acceptance recorded only there is lost to the next selector, the next clone, and any second writer. Mirror the outcome into the row's outcomes-log entry all the same: whether the comparison ran, which out-of-scope paths it found, and how each was accounted for, so a resume is not re-deriving it from a subagent report that is gone.
