...
The operator owns them from there, though no longer alone: `$clean-branches` classifies every local branch on push evidence, so a merged worker branch is now something it can collect rather than pass over. Don't read its verdicts off this report, though — it applies its own rules, and they are not step 6's. A row deferred for **end of run not observed** is the one it plainly collects; a row whose worktree was not clean hits its own dirty-worktree skip; and its squash-path test is a containment where item 1's is a SHA identity, so a row deferred for **branch did not land** may still be collected there. And it deletes only on evidence that the branch itself was pushed, so one that never was — or that went up without `-u` into a repo which deletes heads on merge, or to a remote not named `origin` — is skipped forever, however thoroughly its commits are in the base. Which of your rows fall where is the plan table it prints before its single confirmation, and its own skill — not this paragraph.

**Never point that sweep at a checkout while a campaign or a dispatched worker is in flight**, yours or anyone's — that is the constraint it states for itself. It has no liveness signal, its enumeration is repo-wide, and a clean `git status` says nothing about whether a process is inside a directory, so sweeping while any worker is live removes a worktree its owner is standing in — the failure the observed-end-of-run precondition above exists to prevent, reached by another route. Your rows deferred for **end of run not observed** are the worst case for it, being the ones you could not settle that question on yourself. So keep naming them in the report: the sweep is the operator's move once the campaign is over and nothing is in flight, not a substitute for the report.
