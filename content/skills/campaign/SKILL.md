...
**Verify each issue's PR before merging it.** Green + mergeable says CI passed and Git can fast-forward — neither says the PR contains the work you dispatched. (Your own ADR-index PR below has no issue and no manifest row, so none of this applies to it.)
...
**A hold does not clear itself.** Post `WORK:TRAJECTORY` naming the paths, set `status:needs-human`, mark the row blocked, and keep draining the rest of the queue. The PR stays green and mergeable throughout, so nothing about it will change: only the operator lifts the hold, and step 3's `green + mergeable → ready-to-merge` reconciliation does not apply to a row carrying one. They lift it on the issue, not in the manifest — a comment line reading `Operator-accepted: <path>` accounts for that path on every later pass, and nothing else in the comment does. The manifest is gitignored, slug-keyed and yours alone to write, so it cannot carry a decision a later run or another clone has to read.
