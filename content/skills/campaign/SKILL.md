...
- **The PR must close its assigned issue** — `gh pr view <PR> --json closingIssuesReferences`. A reference to any *other* issue takes the blocker path below: merging closes a row the campaign may still have queued, and a later resume reads that close as already-fixed. A missing reference is recorded and left to the post-merge auto-close check below, which closes the issue explicitly. Either way the reference is necessary, never sufficient — it says which issue the PR claims, not that it implements it. Only reading the PR body, its stated acceptance criteria, and the `WORK:REVIEW` summary settles that, and no command substitutes for that read.
- **Compare `gh pr diff <PR> --name-only` against that issue's `File scope` cell from step 4.** A changed path is in scope when it equals a repo-relative path named in the cell, sits under a directory named there, or sits under the directory holding a glob entry. Use the diff, not `gh pr view --json files`: that field returns only the first 100 paths with no truncation marker, so it reports a clean prefix of exactly the large PRs most worth checking. When the cell is still `—`, holds nothing parseable as a path, or the command fails after one retry, the comparison **did not run** — say so and rely on the read above. Rows reconciled in step 3 and rows from a manifest predating the assignment carry no scope, and treating an absent hint as a mismatch parks correct work.
- **An empty diff blocks.** Nothing was changed, so nothing can be carrying the fix. Treat it as a merge-phase blocker (step 8): post `WORK:TRAJECTORY`, set `status:needs-human`, record the row as blocked, and keep draining the rest of the queue.
- **Changed files outside the scope do not block by themselves.** Step 4 assigns scope as a hint, and a correct fix routinely touches a file the plan didn't predict; a check that hard-blocks on any deviation fires on legitimate work and gets routed around. A file is accounted for when the PR body, a commit message, or the subagent's report ties it to **the assigned issue**. Everything else is **unrelated** — report it and hold that one merge for the operator's decision, via the blocker path above. A file tied to a *different* tracked issue is the case most needing that decision, not an exemption from it: merging it lands a sibling's work early and can auto-close a row still queued, which a later resume then reads as already-fixed. A nonempty diff overlapping nothing in the scope is reported the same way — a probably-stale hint, not proof the PR is wrong. Never split, revert, or cherry-pick inside the PR; that surgery is undefined here and risks discarding work.

Record in that issue's outcomes-log entry whether the file comparison ran and what it found, so a later resume can tell a check that passed from one that never ran.
