...
**Verify before merging.** Green + mergeable says CI passed and Git can fast-forward — neither says the PR contains the work you dispatched. One call answers both halves: `gh pr view <PR> --json closingIssuesReferences,files`.

- **The PR must close the assigned issue.** A missing or mismatched reference is a defect to fix before merging, not to wave through — this step already depends on that link when it verifies auto-close below. It is necessary, never sufficient: the reference says which issue the PR claims, not that it implements it. Only reading the PR body, its stated acceptance criteria, and the `WORK:REVIEW` summary settles that, and no command substitutes for that read.
- **Compare the changed files against that issue's `File scope` cell from step 4.** A changed path is in scope when it equals a repo-relative path named in the cell or sits under a directory named there. When the cell is still `—`, holds nothing parseable as a path, or the query fails after one retry, the comparison **did not run** — say so and rely on the read above. Rows reconciled in step 3 and rows from a manifest predating the assignment carry no scope, and treating an absent hint as a mismatch parks correct work.
