...
Global instructions for all projects. Project-specific instruction files
override these defaults.

- Prefer Exa AI (`mcp__exa__web_search_exa`) over `WebSearch` for all web searches
- **Process skills come first.** Use installed skills proactively when they
  match the task: before designing, use brainstorming; before a bug fix, use
  systematic debugging; before multi-task implementation, write a plan and then
  execute it task-by-task; before claiming done, run verification. If a skill is
  unavailable in the current session, do the work directly and say so briefly.
...
**A review verdict is input, not a deliverable.** Fix defensible findings in the same turn and re-review. Cap loops at ~5 passes; if findings are mostly consequences of your own earlier fixes, or the count isn't falling, stop and escalate.
...
Before
architecture-sensitive generation, build, or verification, run preflight and retain its
recorded host, effective targets, and relationship. Never infer a target from the host or
drop a declared target because it differs from the current machine.
...
| `trash` (mac) / `gio trash` (Linux) | rm | recoverable delete |

### On-demand references

Read the matching reference BEFORE the work it covers:

- **Python** — read the Python reference installed for the current agent
- **Node/TypeScript** — read the TypeScript reference installed for the current
  agent
- **Rust** — read the Rust reference installed for the current agent
- **Bash** — read the Bash reference installed for the current agent
- **GitHub Actions** — read the GitHub Actions reference installed for the
  current agent
- **Dispatching subagents / parallel fan-outs** — read the orchestration
  reference installed for the current agent
